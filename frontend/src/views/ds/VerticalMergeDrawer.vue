...
const mergeAndCreate = async () => {
  ...
  try {
    ...
    // 直接调用后端concatenateExcels接口，返回JSON数据
    const response: any = await datasourceApi.concatenateExcels(formData)
    ...
    const tables = Array.isArray(response.sheets)
      ? response.sheets.map((sheet: any) => ({
          ...
          table_comment: sheet.tableComment || 'Concatenated data'
        }))
      : []
...
<template>
  ...
  >
    <template #header="{ close }">
      <span style="white-space: nowrap">{{ $t('merge.vertical_merge') }}</span>
      <el-icon class="ed-dialog__headerbtn mrt" style="cursor: pointer" @click="close">
        <icon_close_outlined></icon_close_outlined>
      </el-icon>
    </template>

    <div class="vm-form" v-loading="uploading || merging">
          ...
          >
            ...
            <template #tip>
              ...
              <div class="merge-requirements">
                ...
                <ul>
                  <li>{{ $t('merge.vertical_requirement1') }}</li>
                  <li>{{ $t('merge.vertical_requirement2') }}</li>
                  <li>{{ $t('merge.vertical_requirement3') }}</li>
                  <li>{{ $t('merge.vertical_requirement4') }}</li>
                </ul>
              </div>
            </template>
          </el-upload>
