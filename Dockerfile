...
# Provide offline wheel for sqlbot-xpack so uv can resolve via local source
COPY sqlbot_xpack-0.0.3.41-cp311-cp311-manylinux2014_x86_64.whl ${APP_HOME}/

# Final sync to ensure all dependencies are installed (including sqlbot-xpack from local wheel)
...
# Install JRE for JPype1 and Java-dependent features
RUN apt-get update && apt-get install -y --no-install-recommends \
...
# Add Oracle instant client and JVM to ENV
ENV LD_LIBRARY_PATH="/opt/sqlbot/db_client/oracle_instant_client:/usr/lib/jvm/java-17-openjdk-amd64/lib/server:${LD_LIBRARY_PATH}"
